...
class Solver:
    ...
    def get_suggestion_stable(self, k: dict, prev_guesses: tuple) -> str:
        ...
        if k_hash in self.kmap and self.kmap[k_hash][2] >= Knowledge.LOW:
    ...
    def auto_play(self, first_guess: str):
        self.optimize_agts_for_k(first_guess, self.answers, Knowledge.default_knowledge())
    ...
    def auto_play_and_optimize(self, first_guess: str):
        ...
        if menu == 'p': max_guess_to_test = 100
        elif menu == 'f': pass
        else: exit()

        self.optimize_to_update(max_guess_to_test)

        menu = str(input("Optimize one level deeper? y/n:")).lower()
        if menu == 'y':
            deep_update = {}
            for kmap in self.temp_kmap.copy():
                k = copy.deepcopy(self.temp_kmap[kmap]["k"])
                m = Knowledge.get_possible_matches(k, self.answers)
                g = self.temp_kmap[kmap]["g"]
                if self.temp_kmap[kmap]["c"] != Knowledge.PER:
                    deep_update[kmap] = {"g": g, "m": m, "k": k}
            self.temp_kmap = {}
            deep_length = len(deep_update)
            i = 1
            for kmap in deep_update:
                print(str(i), "of", str(deep_length))
                self.optimize_agts_for_k(deep_update[kmap]['g'], deep_update[kmap]['m'], deep_update[kmap]['k'])
                self.optimize_to_update(max_guess_to_test)
                self.temp_kmap = {}
        else: exit()

    def optimize_to_update(self, max_guess_to_test):
        confidence = Knowledge.LOW  # default confidence
        to_update = copy.deepcopy(self.temp_kmap)
        count = 0
        ...
        for k_hash in to_update:
            if to_update[k_hash]["c"] <= Knowledge.MED:  # MED may change with sub queries change. High won't
        ...
        for k_hash in match_list:
            ...
            for guess in match_map:  # use match map to order by top 5 matching guesses
                ...
                i += 1
                agts = self.optimize_agts_for_k(guess, matches, k)
                if best_guess is None or agts < best_agts:
                    best_guess = guess
                    best_agts = agts
                if best_agts <= 2 - 1/len(matches): break

            if best_agts <= 2.0: confidence = Knowledge.HIGH  # found match that is a good guess. move on
            else:
                to_eg = True
                to_ek = True
                existing_kmap = False

                existing_suggestion = self.maps.get_suggestion(k_hash)
                if existing_suggestion: to_eg = False
            
                if k_hash in self.kmap: 
                    existing_kmap = self.kmap[k_hash][0]
                    to_ek = False
                ...
                for guess in match_map:  # use match map to order by top 50 guesses
                    j += 1
                    if guess in matches: continue  # skip matches we've done then!
                    if not to_eg: to_eg = guess == existing_suggestion  # False until seen existing suggestion
                    if not to_ek: to_ek = guess == existing_kmap  # False until seen existing suggestion
                    if to_eg and to_ek: i += 1  # start incrementing match count after current best guess
                    if i > max_guess_to_test: break
                    agts = self.optimize_agts_for_k(guess, matches, k)
                    if best_guess is None or agts < best_agts:
                        best_guess = guess
                        best_agts = agts
                if confidence < Knowledge.HIGH and j == len(matches): confidence = Knowledge.MED
            ...
            if best_agts < self.kmap[k_hash][1]:
                ...
                color = COLORS.GREY
                notice = "not as good as"
                best_guess = self.kmap[k_hash][0]
                best_agts = self.kmap[k_hash][1]
            print(color, str(best_agts), notice, str(self.kmap[k_hash][1]), COLORS.SPACE_COLOR)
            self.kmap[k_hash] = self.kmap_new[k_hash] = (best_guess, best_agts, confidence)
            self.save_maps_to_db()

    def optimize_agts_for_k(self, prev_guess: str, matches: tuple, starting_knowledge: dict):
        if self.temp_kmap is None: self.temp_kmap = {}
        total_matches = len(matches)
        total_guesses = 0
        ...
        for secret in matches:
            ...
            while guess != secret:
                ...
                if try_count == 2:
                    k_hash = Knowledge.dict_hash(k)
                    if k_hash not in self.temp_kmap:
                        self.temp_kmap[k_hash] = {"matches": 0, "total_guesses": 0, "agts": None,
                                                  "g": guess, "k": k}
                    self.temp_kmap[k_hash]["matches"] += 1
                if guess == secret:
                    dist[try_count] += 1
                    if try_count > 1:
                        self.temp_kmap[k_hash]["total_guesses"] += try_count - 1
                        t_g = self.temp_kmap[k_hash]["total_guesses"]
                        t_m = self.temp_kmap[k_hash]["matches"]
                        self.temp_kmap[k_hash]["agts"] = t_g / t_m
                    if self.debug: print("\r\033[K", secret, "in", str(try_count), end="\n")
                    break
                else:
        ...
        for kmap in self.temp_kmap:
            g = self.temp_kmap[kmap]["g"]
            c = self.temp_kmap[kmap]["agts"]
            m = self.temp_kmap[kmap]["matches"]
            if c <= 2 - 1 / m:
                self.temp_kmap[kmap]["c"] = Knowledge.PER
                self.kmap[kmap] = self.kmap_new[kmap] = (g, c, Knowledge.PER)
                kmap_distro["PER"] += 1
            elif kmap in self.kmap and self.kmap[kmap][2] == Knowledge.HIGH:
                self.temp_kmap[kmap]["c"] = Knowledge.HIGH
                ...
                self.temp_kmap[kmap]["c"] = Knowledge.MED
                kmap_distro["MED"] += 1  # avoid re-optimizing previously assessed solutions
            elif kmap in self.kmap and self.kmap[kmap][2] == Knowledge.LOW:
                self.temp_kmap[kmap]["c"] = Knowledge.LOW
                kmap_distro["LOW"] += 1
            else:
                self.kmap[kmap] = self.kmap_new[kmap] = (g, c, Knowledge.VERY_LOW)
                self.temp_kmap[kmap]["c"] = Knowledge.VERY_LOW
                kmap_distro["VERY LOW"] += 1
            count += 1
        if total_guesses > 50:  # supress small options
            print("\r\033[K", prev_guess, total_guesses, avg_f, str(dist), end="\n")
            print(kmap_distro)
        return avg
